...
async function seedPromos() {
    ...
    const existingCodes = new Set(
        existingPromos?.map((p) => p.stripe_coupon_code) || []
    );

    // Filter out promos that already exist
    const promosToInsert = INITIAL_PROMOS.filter(
        (promo) => !existingCodes.has(promo.stripe_coupon_code)
    );
    ...
    data?.forEach((promo) => {
        console.log(
            `   • ${promo.name} (${promo.stripe_coupon_code}) - ${promo.discount_percent}% off`
        );
    });

    console.log(
        `\n✨ Total promos in database: ${
            (existingPromos?.length || 0) + (data?.length || 0)
        }\n`
    );
}
