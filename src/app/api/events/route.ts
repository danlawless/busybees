...
export async function GET() {
  try {
    ...
    const { data, error } = await adminSupabase
      .from('events')
      .select('id, title, description, image_url, event_date, event_date_end, event_time_start, event_time_end, is_free, is_bookable, max_capacity, pass_ids, booking_instructions, toddler_price, infant_price')
      .eq('status', 'published')
