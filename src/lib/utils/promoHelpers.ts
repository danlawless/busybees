...
export function generatePromoMessages(promo: PromoSpecial): string[] {
  ...
  if (daysRemaining > 7) {
    messages.push(`${promo.name} - ${promo.discountPercent}% OFF through ${formatPromoDate(promo.endDate)}`);
  } else if (daysRemaining > 1) {
    messages.push(`Only ${daysRemaining} days left! Save ${promo.discountPercent}%`);
  } else if (daysRemaining === 1) {
    messages.push(`Last chance! Only ${hoursRemaining} hours remaining`);
  } else if (hoursRemaining > 0) {
    messages.push(`Final hours! ${hoursRemaining} hours left to save ${promo.discountPercent}%`);
  }

  messages.push(`Use code ${promo.stripeCouponCode} at checkout`);
  messages.push(`Don't miss out! ${promo.name} ends ${formatPromoDate(promo.endDate)}`);
 ...
 */
export function validatePromoDates(startDate: string, endDate: string): { valid: boolean; error?: string } {
